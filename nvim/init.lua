-- Vanilla Configuration

require('user.options')                                   -- Load general settings
require('user.plugins')                                   -- Define and install plugins
require('user.colorscheme')                               -- Set colorscheme
require('user.keymaps')                                   -- Define keymaps
require('user.lualine')                                   -- Lualine configuration
require('user.nvimtree')                                  -- Nvim Tree configuration
require('user.vimwiki')                                   -- Vimwiki configuration
require('user.markdown-preview')                          -- Markdown Preview configuration
